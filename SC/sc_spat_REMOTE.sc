...
s = Server.local(\beatspat_client, NetAddr("127.0.0.1", 57140));


s.options.device = "BEATSPAT_remote";
		...
		~sliders_MOVEMENTS[i].action_(
			{
				...
				~sendOSC.sendMsg("/route/spatial", ~my_IDX, i, ~sliders_MOVEMENTS[i].value);
		});
		...
		~sliders_SPEAKERS[i].action_(
			{
				...
				~sendOSC.sendMsg("/route/spatial", ~my_IDX, i+10, ~sliders_SPEAKERS[i].value);
		});
